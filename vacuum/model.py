...
from vacuum.io_ import deprocess
...
def create_generator(generator_inputs, generator_outputs_channels, ngf, separable_conv):
    ...
    with tf.variable_scope("encoder_1"):
        output = gen_conv(generator_inputs, out_channels=ngf, separable_conv=separable_conv)
...
def create_model(
    ...
    with tf.variable_scope("generator"):
        out_channels = 1
        outputs = create_generator(input_, out_channels, ngf, separable_conv)
    ...
    with tf.name_scope("real_discriminator"):
        with tf.variable_scope("discriminator"):
            ...
            predict_real = create_discriminator(input_, targets, ndf)

    with tf.name_scope("fake_discriminator"):
        with tf.variable_scope("discriminator", reuse=True):
            # 2x [batch, height, width, channels] => [batch, 30, 30, 1]
            predict_fake = create_discriminator(input_, outputs, ndf)
